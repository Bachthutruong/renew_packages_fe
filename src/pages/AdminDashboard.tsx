...
const AdminDashboard: React.FC = () => {
  ...
  const handleB1Select = async (value: string) => {
    ...
    try {
      console.log(`Loading B2 data for B1: ${value}`);
      const data = await dataAPI.getB2Data(value);
      console.log(`Loaded ${data.length} B2 items for B1: ${value}`);
      setB2Data(data);
    } catch (error: any) {
  ...
  const handleMoveToB3 = async () => {
    ...
    try {
      console.log(`Loading B3 data for B1: ${selectedB1}, B2: ${selectedB2}`);
      const data = await dataAPI.getB3Data(selectedB1, selectedB2);
      console.log(`Loaded ${data.length} B3 items`);
      setB3Data(data);
    } catch (error: any) {
